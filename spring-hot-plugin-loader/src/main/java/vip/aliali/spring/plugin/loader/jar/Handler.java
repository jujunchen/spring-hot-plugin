...
package vip.aliali.spring.plugin.loader.jar;
...
public class Handler extends URLStreamHandler {
	...
	protected URLConnection openConnection(URL url) throws IOException {
		if (this.jarFile != null && isUrlInJarFile(url, this.jarFile)) {
			return vip.aliali.spring.plugin.loader.jar.JarURLConnection.get(url, this.jarFile);
		}
		try {
			return vip.aliali.spring.plugin.loader.jar.JarURLConnection.get(url, getRootJarFileFromUrl(url));
		}
		catch (Exception ex) {
		...
		if (canResetCachedUrlHandlers()) {
			...
			try {
				...
				try {
					...
					if (connection instanceof vip.aliali.spring.plugin.loader.jar.JarURLConnection) {
						jarContextUrl = null;
					}
				}
				catch (Exception ex) {
