...
基于Spring的 applicationContext 和 classLoader 对插件中的类进行热加载，卸载的时候尽可能切断GC ROOT，避免内存泄露。
...
## 性能测试
测试用例：
模拟实际频繁的插件安装卸载和增删改数据操作，观察内存消耗情况。

Docker 容器最大内容：256MB

1. 安装plugin-demo-mybatis插件
2. 新增数据
3. 查询数据
4. 删除数据
5. 卸载plugin-demo-mybatis插件
